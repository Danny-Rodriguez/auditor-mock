import { BadgeDelta, Card, DeltaType, DonutChart, Dropdown, DropdownItem, Flex, Legend, List, ListItem, Title } from "@tremor/react";
...
const filterByRegion = (region: string, data: CityData[]) => (region === "all" ? data : data.filter(city => city.region === region));
  ...
  return (
    <Card className="">
      <Flex className="space-x-8" justifyContent="start" alignItems="center">
        <Title>Sales</Title>
        <Dropdown onValueChange={value => setSelectedRegion(value)} placeholder="Region Selection">
          {regions.map(region => (
